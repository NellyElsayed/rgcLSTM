...
from keras_utils_rgcLSTM import legacy_pred_rgcLSTM_support
import numpy as np
from keras.layers import Dot
class Pred_rgcLSTM(Recurrent):
    '''Pred-rgcLSTM model: This code is based on:
    PredNet architecture - Lotter 2016.
    As the first desinger and author of this code. 
   
    This model modified by: Nelly Elsayed to use the reduced-gate convolutional LSTM (rgcLSTM) instead of the convolutional LSTM (cLSTM)
    used in PredNet architecture - Lotter 2016 .
    
    Therfore we named our model: Pred-rgcLSTM
    We preserved the naming of parameters the same as the original PredNet, and modified and added the required 
    statments to replace the (cLSTM)
    The following comment also based on Lotter 2016 source code:
    ...
    # new_c:
    used as a case to the activation weight handling instead of the one used by the original code, to convert the model to rgcLSTM instead of cLSTM
    Please read the comments within the code to appl;y changes if you switches between GRB and gray scale images
    '''
    @legacy_pred_rgcLSTM_support
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        super(Pred_rgcLSTM, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        self.input_spec = [InputSpec(shape=input_shape)] #size (None, batch_size, 64,64)
        self.conv_layers = {c: [] for c in ['f', 'a', 'ahat', 'new_c']}
        ...
        for c in sorted(self.conv_layers.keys()):
            for l in range(len(self.conv_layers[c])):# There the weight base on the RGB input for KITTI dataset
                ds_factor = 2 ** l
                if c == 'ahat':
                    ...
                    if l == 0:
                        nb_channels = 57# the value should be changed to 51 in the moving MNIST video.
                    elif l == 1:
                        nb_channels = 240
                    elif l == 2:
                        nb_channels = self.R_stack_sizes[l]*5
                    elif l == 3:
                        nb_channels = self.R_stack_sizes[l]*3
                    print('nb_channels= ', nb_channels) 
                    print("l= ", l)
                    self.counter1+=2
                else:
                    nb_channels = self.stack_sizes[l] * 3 + self.R_stack_sizes[l]
                    if l < self.nb_layers - 1:
  ...
  #      ########added
        ...
        for l in reversed(range(self.nb_layers)):
            inputs = [r_tm1[l], e_tm1[l], c_tm1[l]]
            temp = c_tm1[l]
         #   temp1= e_tm1[l]
            temp=temp*0
           # temp=temp+1
            inputs1 = [r_tm1[l], e_tm1[l], temp]
            inputs2 = [r_tm1[l], e_tm1[l]]
            if l < self.nb_layers - 1:
                inputs.append(r_up)
                inputs1.append(r_up)
                inputs2.append(r_up)
            print("inputs gate: ",inputs)
            print("inputs activation: ",inputs2)
            inputs = K.concatenate(inputs, axis=self.channel_axis)
            inputs1 = K.concatenate(inputs1, axis=self.channel_axis)
            inputs2 = K.concatenate(inputs2, axis=self.channel_axis)
            
            f = self.conv_layers['f'][l].call(inputs) 
            temp2 = self.conv_layers['new_c'][l].call(inputs2)

            _c = f* c_tm1[l] + f*temp2 
            _r = f * self.LSTM_activation(_c)
        ...
        for l in range(self.nb_layers):
            ...
            #e.append(K.concatenate((ahat, a), axis=self.channel_axis))
    ...
    def get_config(self):
        ...
        base_config = super(Pred_rgcLSTM, self).get_config()
