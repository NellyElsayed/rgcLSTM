'''
Evaluate trained Pred_rgcLSTM on KITTI sequences. based on the PredNet modified code.
Calculates mean-squared error and plots predictions.
The original code is done by Lotter 2016 implementation
...
from pred_rgcLSTM import Pred_rgcLSTM
...
weights_file = os.path.join(WEIGHTS_DIR, 'pred_rgcLSTM_kitti_weights.hdf5')# change to Moving MNIST  file if you use Moving MNIST
json_file = os.path.join(WEIGHTS_DIR, 'pred_rgcLSTM_kitti_model.json')# change to Moving MNIST file if you use Moving MNIST
...
test_pred_rgcLSTM = Pred_rgcLSTM(weights=train_model.layers[1].get_weights(), **layer_config)
